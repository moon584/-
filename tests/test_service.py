...
def test_persist_record_refreshes_metadata_when_content_unchanged() -> None:
    ...
    crawler = JobCrawler(fake_db, http_client=object(), provider=_FakeProvider(), job_type=0)
...
def test_resolve_category_mappings_uses_default_when_database_empty() -> None:
    ...
    crawler = JobCrawler(fake_db, http_client=object(), provider=provider, job_type=0)
